...
var validators = Object.freeze({
  ...
  var Validator$1 = function () {
    ...
    Validator.prototype.waitFor = function waitFor(name, cb) {
      var method = '$activateValidator' + '_' + name;
      var cbs = {};
      var vm = this._dir.vm;
      cbs[name] = cb;
      vm[method] = function () {
    ...
    Validator.prototype._defineResetValidation = function _defineResetValidation() {
      var vm = this._dir.vm;
      this._dir.vm.$resetValidation = function (name, cb) {
        vm._validatorMaps[name]._resetValidation(cb);
      };
    };
    ...
    Validator.prototype._defineValidate = function _defineValidate() {
      ...
      this._dir.vm.$validate = function () {
        ...
        vm._validatorMaps[name].validate({ field: field, touched: touched, cb: cb });
      };
    };

    Validator.prototype._defineSetValidationErrors = function _defineSetValidationErrors() {
      var vm = this._dir.vm;
      this._dir.vm.$setValidationErrors = function (name, errors) {
        vm._validatorMaps[name]._setValidationErrors(errors);
      };
    };
  ...
  function Validator (Vue) {
    ...
    Vue.elementDirective('validator', {
      ...
      setupFragment: function setupFragment(lazy) {
        var _this = this;
        var name = this.validatorName.replace('$', '');
        var vm = this.vm;
        vm.$activateValidator = function (_name) {
          vm['$activateValidator' + '_' + _name.replace('$', '')]();
        };

        this.validator.waitFor(name, function () {
        ...
        !lazy && vm['$activateValidator' + '_' + name]();
      },
      teardownFragment: function teardownFragment() {
